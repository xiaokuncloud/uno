...
(function () {
  ...
  function cardHTML(card, opts = {}) {
    ...
    if (opts.faceDown) {
      cls.push('face-down');
      return `<div class="${cls.join(' ')}"><img src="/assets/cards/back.webp?v=29" alt=""></div>`;
    }
    cls.push(cardClass(card));
    if (opts.highlight) cls.push('highlight');
    if (opts.disabled) cls.push('disabled');
    return `<div class="${cls.join(' ')}"><img src="/assets/cards/${cardImg(card)}?v=29" alt=""></div>`;
  }
  ...
  function preloadCards() {
    ...
    files.forEach((f) => { const im = new Image(); im.src = '/assets/cards/' + f + '?v=29'; });
  }
  ...
  function speak(text) {
    if (localStorage.getItem('uno_bgm_off') === '1') return;
    const now = Date.now();
  ...
  function updateHintModeUI() {
    ...
    const b2 = $('btn-hint-mode-2');
    if (b2) b2.textContent = S.hintMode === 'auto' ? '自动' : '手动';
    const hb = $('btn-hint');
    if (hb) hb.classList.toggle('hidden', S.hintMode === 'auto');
  }
  function updateHintModeUI2() { updateHintModeUI(); }
  function applyHandHint() {
    ...
    const canHint = S.hintOn && ui.phase === 'playing' && myTurn;
    Array.from(handEl.children).forEach((wrap, i) => {
      const card = S.lastHand[i];
      const c = wrap.querySelector('.uno-card');
      if (!card || !c) return;
      const canPlay = canHint && UnoCore.canPlay(card, ui.discardTop, ui.chosenColor);
      // 默认全部变淡（无高亮）；点提示且能出时恢复鲜艳（高亮）
  ...
  function clearHandHint(handEl) {
    Array.from(handEl.querySelectorAll('.uno-card')).forEach((c) => {
      c.classList.remove('highlight');
      c.classList.add('disabled'); // 默认全部变淡
    });
  }
  ...
  function showColorPicker(cb) {
    ...
    // 禁止：不出这张牌（和颜色项同结构：圆点+文字，对齐）
    const no = document.createElement('div');
    no.className = 'color-item no-color';
    const noDot = document.createElement('div');
    noDot.className = 'dot';
    noDot.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="#e63946" stroke-width="2.4" stroke-linecap="round"><circle cx="12" cy="12" r="9"/><line x1="5" y1="5" x2="19" y2="19"/></svg>';
    const noLabel = document.createElement('span');
    noLabel.className = 'color-label';
    noLabel.textContent = '不出';
    no.appendChild(noDot);
  ...
  window.doSurrender = doSurrender;
  updateHintModeUI();
  ...
  function connectOnline() {
    ...
    S.ws.onopen = () => {
      ...
      let rejoinIdx;
      if (sameRoom) {
        // 同一房间重连：带 selfIndex 精确重连（房主0也允许，防"房间已满"）
        rejoinIdx = Number(savedIdx);
      }
  ...
  function handleOnlineMsg(m) {
    switch (m.action) {
      ...
      case 'peerJoined':
        ...
        speak((m.playerNames && m.playerNames[1]) + '加入了房间');
        break;
      case 'error':
        if (S.reconnecting) {
        ...
        break;
      case 'state':
        if (!S._gameActive) { S._gameActive = true; S.hintOn = false; syncHintBtn(); }
        S.ui = m.game;
        ...
        // 定庄阶段：显示抽牌定庄弹窗
        if (S.ui.phase === 'drawing') { showDealerModal(S.ui); S._winSpoken = false; }
  ...
  function drawUI(ui) {
    ...
    for (let i = 0; i < Math.min(n, 24); i++) {
      ...
      mc.innerHTML = '<img src="/assets/cards/back.webp?v=29" alt="">';
    ...
    const handEl = $('my-hand');
    const myHandRaw = ui.selfHand || [];
    const COLOR_ORDER = { red: 0, yellow: 1, green: 2, blue: 3, wild: 4 };
    const myHand = [...myHandRaw].sort((a, b) => {
      const ca = COLOR_ORDER[a.color] != null ? COLOR_ORDER[a.color] : 5;
      const cb = COLOR_ORDER[b.color] != null ? COLOR_ORDER[b.color] : 5;
    ...
    // 自动提示模式：轮到我出牌时自动高亮；无牌可出时提示（每回合只提示一次）
    if (S.hintMode === 'auto') {
      if (ui.phase === 'playing' && myTurn) {
        S.hintOn = true;
        syncHintBtn();
        const hasPlayable = S.lastHand && S.lastHand.some((c) => UnoCore.canPlay(c, ui.discardTop, ui.chosenColor));
        if (!hasPlayable && !S._autoNoCardHinted) {
          S._autoNoCardHinted = true;
          toast('无牌可出，请抽牌');
          speak('无牌可出，请抽牌');
        }
      } else {
    ...
    if (ui.phase === 'ended' && S.mode !== 'solo') {
      ...
      if (!S._winSpoken) { S._winSpoken = true; speak(ui.winner === self ? '你赢了' : '对方获胜'); }
      showResult(
  ...
  function doSurrender() {
    if (S.mode === 'solo') {
      ...
      g.scores[1] = (g.scores[1] || 0) + 1;
      addLog('你认输了，AI 获胜', 'win');
      soloRender();
      showResult('AI 获胜', '你认输了。当前比分 ' + g.scores[0] + ':' + g.scores[1]);
      speak('你认输了');
    } else {
      sendWs({ action: 'surrender' });
    }
    closeSettings();
  }
  function goHome() {
  ...
  function soloNewGame() {
    const prevScores = (S.solo && S.solo.scores) ? S.solo.scores : [0, 0];
    clearSoloState();
    S.solo = UnoCore.createGame([S.name, 'AI']);
    S.solo.scores = prevScores;
    S.hintOn = false;
    syncHintBtn();
    S.log.length = 0;
  ...
  function aiActionFx() {
    ...
    if (act.type === 'play') {
      ...
      const clone = document.createElement('div');
      clone.className = 'uno-card face-down';
      clone.innerHTML = '<img src="/assets/cards/back.webp?v=29" alt="">';
...
(function () {
  ...
  function playTry() {
    if (!bgm.getAttribute('src')) bgm.setAttribute('src', '/assets/bgm.mp3?v=29');
